...
import { Button, Divider, List, ListItem, ListItemText, makeStyles, Typography } from '@material-ui/core';
...
function App() {
  ...
  const eliminarTipo = async(tipo) => {
    const resp = await axios.delete(`https://localhost:5001/api/BajaTipoMedicamento?tipo=${tipo}`)
    console.log(resp)
    traerTiposMedicamento();
  }
  ...
  return (
    <div className="container">
      ...
      <Divider></Divider>
      <br/>
      <Typography  variant="h3" component="h7">Eliminar tipos de medicamento</Typography>
      <List dense className={classes.root} width>
      {tipos.map((value) => 
          <ListItem  button>
            <ListItemText id={value.id} primary={value.tipo} /> 
            <Button onClick={()=>eliminarTipo(value.tipo)}><DeleteIcon/></Button>
          </ListItem>
        )}
    </List>
    </div>
